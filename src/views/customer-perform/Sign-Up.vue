...
import { Eye, EyeOff, ArrowRight, CheckCircle, ShieldCheck, Lock, Mail, User, Smile, BadgeCheck, Star } from 'lucide-vue-next';
...
// Giả lập check độ mạnh mật khẩu
const passwordStrength = computed(() => {
  if (password.value.length === 0) return 0;
  if (password.value.length < 6) return 1; // Yếu
  if (password.value.length < 10) return 2; // Khá
  return 3; // Mạnh
});
</script>
...
<template>
  <div class="relative h-screen w-full font-sans text-black overflow-hidden bg-[#FDFBF7] selection:bg-[#A78BFA] selection:text-black">
    ...
    <div class="flex h-full w-full flex-wrap">
      ...
      <div class="relative flex h-full w-full flex-col justify-center overflow-y-auto bg-[#FDFBF7] p-6 lg:w-5/12 lg:p-10 xl:w-4/12 scrollbar-hide z-10">
        
        <div class="absolute top-4 right-4 rotate-6 border-2 border-black bg-[#00C16A] px-3 py-1 font-mono text-xs font-bold text-white shadow-[2px_2px_0px_0px_#000]">
          JOIN_US_NOW
        </div>
        ...
        <div class="mx-auto w-full max-w-sm mt-10 lg:mt-0">
          ...
          <div class="mb-6">
            <div class="flex items-center gap-2 mb-2">
                <Smile class="h-8 w-8 text-black" />
            ...
            <p class="font-mono text-xs font-bold text-gray-500 mt-2">
          ...
          <form class="space-y-4">
            ...
            <div class="space-y-1">
              <label class="font-mono text-xs font-black uppercase flex items-center gap-1">
                <Mail class="h-3 w-3" /> Email đăng ký
              </label>
              <input 
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-1 col-span-2 md:col-span-1">
                    <label class="font-mono text-xs font-black uppercase flex items-center gap-1">
                        <BadgeCheck class="h-3 w-3" /> Xác nhận
                    </label>
                    <input 
          ...
          <p class="mt-6 text-center font-mono text-xs font-bold text-gray-500">
            Đã có tài khoản? 
            <a href="/dang-nhap" class="bg-black text-white px-1 py-0.5 hover:bg-[#A78BFA] hover:text-black transition-colors">Đăng nhập</a>
          </p>

        </div>
      </div>

      <div class="relative hidden h-full flex-col justify-between border-l-[8px] border-black bg-[#A78BFA] p-8 overflow-hidden lg:flex lg:w-7/12 xl:w-8/12">
                 ...
                 <div class="relative w-72 rounded-xl border-4 border-black bg-white p-6 shadow-[10px_10px_0px_0px_#000] transition-all duration-300 group-hover:rotate-2 group-hover:scale-105 z-10">
                    ...
                    <div class="flex flex-col items-center border-b-2 border-dashed border-black pb-4 mb-4">
                        <div class="h-24 w-24 rounded-full border-4 border-black bg-[#FDFBF7] mb-3 overflow-hidden">
                             <img src="https://ui-avatars.com/api/?name=New+User&background=random&size=128" />
                        </div>
                        <h3 class="font-archivo text-2xl font-black uppercase">{{ name || 'YOUR NAME' }}</h3>
