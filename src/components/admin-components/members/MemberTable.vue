<template>
  <NeoCard class="overflow-hidden flex flex-col min-h-[400px]">
    <div v-if="selectedIds.length > 0" class="bg-yellow-100 border-b-2 border-black p-3 flex justify-between items-center animate-fade-in">
       <span class="font-bold text-sm ml-2">Đã chọn {{ selectedIds.length }} thành viên</span>
       <div class="flex gap-2">
          <button @click="$emit('bulk-delete')" class="text-xs font-bold border-2 border-black bg-white px-3 py-1 rounded hover:bg-red-50 text-red-600">
             <i class="fa-solid fa-trash mr-1"></i> Xóa
          </button>
          <button @click="$emit('clear-selection')" class="text-xs font-bold underline px-3 py-1">Hủy</button>
       </div>
    </div>
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-black text-white border-b-2 border-black">
            <th class="p-4 w-10">
              <input type="checkbox" :checked="isAllSelected" @change="$emit('toggle-all')" class="w-4 h-4 accent-neo-green cursor-pointer">
            </th>
        ...
        <tbody>
          <tr v-for="user in users" :key="user.id" class="border-b-2 border-black last:border-0 hover:bg-gray-50 transition-colors group">
            <td class="p-4">
              <input 
                type="checkbox" 
            ...
            <td class="p-4 text-right">
              <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button @click="$emit('edit', user)" class="w-8 h-8 border-2 border-black rounded hover:bg-black hover:text-white transition-colors" title="Sửa">
                      <i class="fa-solid fa-pen text-xs"></i>
                  </button>
                  <button @click="$emit('delete', user.id)" class="w-8 h-8 border-2 border-black rounded hover:bg-red-600 hover:text-white hover:border-red-600 transition-colors" title="Xóa">
...
const emit = defineEmits(['edit', 'delete', 'toggle-all', 'update:selectedIds', 'bulk-delete', 'clear-selection', 'page-change']);

const toggleSelection = (id) => {
  const newSelection = [...props.selectedIds];
  if (newSelection.includes(id)) {
    emit('update:selectedIds', newSelection.filter(item => item !== id));
  } else {
